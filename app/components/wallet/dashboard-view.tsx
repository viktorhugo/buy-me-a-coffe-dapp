import { Coffee, Info, PackageOpen, Wallet } from "lucide-react";
...
import { ContractFunctionRevertedError, BaseError, parseEther, formatEther, type Abi, type SimulateContractParameters } from "viem";
...
function ConnectedView() {
    ...
    const { publicClient, walletClient, walletBalance, contractBalance } = useWalletStore((state) => state);
    ...
    async function fund() {
        ...
        try {
            ...
            console.log('simulateContractParameters', parameters);
    ...
    async function withdraw() {
        ...
        const simulateContractParameters: SimulateContractParameters = {
            address: contractAddress,
            abi: abi as Abi,
            functionName: 'fund',
            args: [connectedAccount],
        ...
        console.log('simulateContractParameters', simulateContractParameters);
        const response = await publicClient.simulateContract(simulateContractParameters);
        ...
        <>
            <div className="text-center justify-center">
                ...
                <Card className="bg-slate-900/50 border-slate-800 w-[450px] mx-auto">
                    <CardHeader>
                        <CardTitle className="text-2xl text-white">Create project</CardTitle>
                        <CardDescription className="text-sm text-slate-400">Address Contract</CardDescription>
                        <CardDescription className="text-sm text-slate-200">{contractAddress}</CardDescription>
                        <CardDescription className="text-sm text-slate-400">Wallet Balance: { walletBalance ? Number(formatEther(BigInt(walletBalance.toString()))) : 0 } ETH</CardDescription>
                        <CardDescription className="text-sm text-slate-400">Contract Balance: { contractBalance ? Number(formatEther(BigInt(contractBalance.toString()))) : 0 } ETH</CardDescription>
                    </CardHeader>
                    <Form method="post" className="flex flex-col gap-4">
                        ...
                        <CardFooter className="flex justify-end-safe gap-2">
                            ...
                            variant="outline"
                            className="cursor-pointer bg-blue-500 border-blue-500 text-white hover:bg-blue-400 hover:border-blue-400 hover:text-white">
                                <PackageOpen className="mr-2 size-4" />
                                Withdraw
                            </Button>
                            <Button onClick={ () => fund() }
                                    variant="outline"
                                    className="cursor-pointer bg-amber-300 text-black border-amber-300 hover:bg-amber-200 hover:border-amber-200">
